...
export function creditsFor(usage: TokenUsage): number {
  const raw =
    (usage.input / 1000) * CREDITS_PER_1K_INPUT + (usage.output / 1000) * CREDITS_PER_1K_OUTPUT;
 ...
 * at settlement.
 */
export function estimateCredits(promptText: string, maxOutputTokens: number): number {
