 ...
 */
import { splitTerminal, type Project } from '@robo-journey/parts';
 ...
 */
function describeNets(project: Project): string {
  ...
  for (const terminal of [project.wires.flatMap((w) => [w.from, w.to])].flat()) {
    if (!parent.has(terminal)) parent.set(terminal, terminal);
  }
  for (const wire of project.wires.slice(0, MAX_WIRES)) union(wire.from, wire.to);
  ...
  if (lines.length === 0) return '  (nothing is wired together yet)';
  return lines.join('\n');
}

function describeParts(project: Project): string {
  if (project.parts.length === 0) return '  (the canvas is empty)';
 ...
 */
function describeVoltages(context: WorkspaceContext): string {
 ...
 */
export function describeWorkspace(context: WorkspaceContext): string {
