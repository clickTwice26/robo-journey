...
import { describeWorkspace } from './context.js';
import { estimateCredits, creditsFor } from './pricing.js';
/**
 * Default model.
 *
...
export function estimateChatCredits(request) {
...
export async function ask(request) {
    ...
    try {
        ...
        throw new AssistantError(`The assistant could not answer: ${error instanceof Error ? error.message : String(error)}`);
    }
    const answer = response.text?.trim() ?? '';
    if (!answer)
        throw new AssistantError('The assistant returned nothing.');
